...
const formatSample = (sample) => {
    return sample.map((line) => {
        ...
        return { input: splitLine[0].trim(), output: splitLine[1].trim() };
    });
};

const countUniqueDigits = (sample) => {
    let count = 0;
...
const extractDigit = (line, length) => {
    const output = [];
...
const findSegments = (line) => {
    const one = extractDigit(line.input, 2)[0];
    const seven = extractDigit(line.input, 3)[0];
    const four = extractDigit(line.input, 4)[0];

    const segmentA = seven
        .split('')
        .filter((segment) => !one.includes(segment))[0];

    const twoThreeFive = extractDigit(line.input, 5);

    const three = twoThreeFive.filter((digit) =>
        one.split('').every((segment) => digit.includes(segment))
    )[0];

    const segmentG = three
        .split('')
        .filter((segment) => !seven.includes(segment))
        .filter((segment) => !four.includes(segment))[0];

    const segmentD = three
        .split('')
        .filter((segment) => !seven.includes(segment))
        .filter((segment) => segment != segmentG)[0];

    const segmentB = four
        .split('')
        .filter((segment) => !seven.includes(segment))
        .filter((segment) => segment != segmentD)[0];

    const five = twoThreeFive.filter((digit) => digit.includes(segmentB))[0];

    const two = twoThreeFive.filter(
        (digit) => digit !== three && digit !== five
    )[0];

    const segmentC = one
        .split('')
        .filter((segment) => !five.includes(segment))[0];

    const segmentF = one
        .split('')
        .filter((segment) => !two.includes(segment))[0];
...
const count = countUniqueDigits(sampleFormatted);

let total = 0;

sampleFormatted.forEach((line) => {
    const segments = findSegments(line);
    let number = 0;

    line.output.split(' ').forEach((digit) => {
        const remappedSegments = digit
            .split('')
            .map((segment) => segments[segment])
            .sort((a, b) => (a > b ? 1 : -1))
            .join('');

        number = number * 10 + digits[remappedSegments];
    });
