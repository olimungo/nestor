...
const getValue = (board, line) => {
    return board[line] === undefined ? 0 : board[line];
};
...
const processInput = (input) => {
    ...
    boards.forEach((board, index) => {
        ...
        for (let indexLine = 0; indexLine < 5; indexLine++) {
            boards[index].push([
                -1,
                getValue(boards[index][0], indexLine),
                getValue(boards[index][1], indexLine),
                getValue(boards[index][2], indexLine),
                getValue(boards[index][3], indexLine),
                getValue(boards[index][4], indexLine),
            ]);
        }
    });
...
const checkWinner = (board) => {
    for (let index = 0; index < board.length; index++) {
        const line = board[index].filter((item) => item > 0);
...
const play = (boards, numbers) => {
    while (numbers.length > 0) {
        ...
        boards.forEach(
            (board, index) =>
                (boards[index] = board.map((line) =>
                    line.filter((item) => item != number)
                ))
        );
        ...
        for (let board = boards.length - 1; board >= 0; board--) {
            if (checkWinner(boards[board])) {
                if (boards.length == 1) {
                    return { board: boards[board], index: board, number };
                }
    ...
    return { board: [], index: -1, number: -1 };
};

const boards = processInput(sample);
const result = play([...boards], [...numbers]);
...
if (result.board.length > 0) {
    ...
    const total = board.reduce(
        (total, line) =>
            (total += line.reduce((total, item) => (total += item), 0)),
        0
    );
